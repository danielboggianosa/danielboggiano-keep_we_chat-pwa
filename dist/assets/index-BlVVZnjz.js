(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&n(s)}).observe(document,{childList:!0,subtree:!0});function t(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(a){if(a.ep)return;a.ep=!0;const r=t(a);fetch(a.href,r)}})();async function O(){if(!("serviceWorker"in navigator)){console.warn("Service Workers not supported in this browser.");return}try{const i=await navigator.serviceWorker.register("/sw.js",{scope:"/"});return console.log("Service Worker registered with scope:",i.scope),i}catch(i){console.error("Service Worker registration failed:",i);return}}const B="meeting-transcription-db",_=1,E={AUDIO_FILES:"audioFiles",TRANSCRIPTIONS:"transcriptions",SYNC_QUEUE:"syncQueue",SETTINGS:"settings"};function z(){return new Promise((i,e)=>{const t=indexedDB.open(B,_);t.onupgradeneeded=n=>{const a=n.target.result;j(a)},t.onsuccess=n=>{i(n.target.result)},t.onerror=n=>{e(n.target.error)}})}function j(i){if(!i.objectStoreNames.contains(E.AUDIO_FILES)){const e=i.createObjectStore(E.AUDIO_FILES,{keyPath:"id"});e.createIndex("recordedAt","recordedAt",{unique:!1}),e.createIndex("syncStatus","syncStatus",{unique:!1})}if(!i.objectStoreNames.contains(E.TRANSCRIPTIONS)){const e=i.createObjectStore(E.TRANSCRIPTIONS,{keyPath:"id"});e.createIndex("ownerId","ownerId",{unique:!1}),e.createIndex("status","status",{unique:!1}),e.createIndex("recordedAt","recordedAt",{unique:!1})}if(!i.objectStoreNames.contains(E.SYNC_QUEUE)){const e=i.createObjectStore(E.SYNC_QUEUE,{keyPath:"id",autoIncrement:!0});e.createIndex("priority","priority",{unique:!1}),e.createIndex("type","type",{unique:!1})}i.objectStoreNames.contains(E.SETTINGS)||i.createObjectStore(E.SETTINGS,{keyPath:"key"})}function q(){if(document.getElementById("app-styles"))return;const i=document.createElement("style");i.id="app-styles",i.textContent=`
  ...
  `,document.head.appendChild(i)}const S=(i,e=22)=>`<svg xmlns="http://www.w3.org/2000/svg" width="${e}" height="${e}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${i}</svg>`,y={bell:S('<path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/>'),user:S('<path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>'),search:S('<circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>'),house:S('<path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>'),calendar:S('<rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/>'),settings:S('<path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/>'),mic:S('<path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" x2="12" y1="19" y2="22"/>'),arrowLeft:S('<path d="m12 19-7-7 7-7"/><path d="M19 12H5"/>',20),chevronRight:S('<path d="m9 18 6-6-6-6"/>',18),pause:S('<rect width="4" height="16" x="6" y="4"/><rect width="4" height="16" x="14" y="4"/>',24),square:S('<rect width="18" height="18" x="3" y="3" rx="2"/>',28),flag:S('<path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" x2="4" y1="22" y2="15"/>',24),share:S('<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" x2="15.42" y1="13.51" y2="17.49"/><line x1="15.41" x2="8.59" y1="6.51" y2="10.49"/>'),download:S('<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>')};function H(i){const e=document.createElement("div");e.className="screen active";function t(s){if(s<60)return`${Math.round(s)}s`;const o=Math.floor(s/60);return o<60?`${o} min`:`${Math.floor(o/60)}h ${o%60}min`}function n(s){const o=new Date,c=o.getTime()-s.getTime(),d=864e5;return c<d&&o.getDate()===s.getDate()?`Hoy, ${s.toLocaleTimeString("es",{hour:"2-digit",minute:"2-digit"})}`:c<2*d?`Ayer, ${s.toLocaleTimeString("es",{hour:"2-digit",minute:"2-digit"})}`:s.toLocaleDateString("es",{weekday:"short",hour:"2-digit",minute:"2-digit"})}function a(){const s=new Date().getHours();return s<12?"Buenos días":s<18?"Buenas tardes":"Buenas noches"}function r(){const s=i.getMeetings(),o=s.length,c=s.reduce((p,f)=>p+f.duration,0)/3600,d=s.filter(p=>p.status==="processing").length,u=s.length>0?s.slice(0,10).map(p=>`
      ...
      <div class="header">
        <div class="header-title-wrap">
          <span class="header-greeting">${a()}</span>
    ...
    `,e.querySelector("#record-fab").addEventListener("click",i.onRecordClick),e.querySelector(".search-inner").addEventListener("click",i.onSearchClick),e.querySelectorAll(".meeting-card[data-meeting-id]").forEach(p=>{p.addEventListener("click",()=>{const f=p.dataset.meetingId;i.onMeetingClick(f)})})}return r(),{element:e,refresh:r}}function V(i){const e=document.createElement("div");return e.textContent=i,e.innerHTML}function U(i){const e=document.createElement("div");e.className="screen",e.style.cssText="padding-bottom:0;";const t=15;e.innerHTML=`
    <!-- Header -->
  ...
  `;const n=e.querySelector("#rec-timer"),r=e.querySelector("#rec-waveform").querySelectorAll(".bar"),s=e.querySelector("#rec-transcript-lines");e.querySelector("#rec-back").addEventListener("click",i.onBack),e.querySelector("#rec-stop").addEventListener("click",i.onStop),e.querySelector("#rec-pause").addEventListener("click",i.onPause),e.querySelector("#rec-flag").addEventListener("click",i.onFlag);function o(f){const b=Math.floor(f/3600),x=Math.floor(f%3600/60),w=f%60;return[b,x,w].map(k=>k.toString().padStart(2,"0")).join(":")}function c(){n.textContent="00:00:00",s.innerHTML="",r.forEach(f=>{f.style.height="20px"})}function d(f){n.textContent=o(f)}function u(){r.forEach(f=>{const b=10+Math.random()*50;f.style.height=`${b}px`})}function p(f,b,x){const w=document.createElement("div");for(w.className="rec-lt-line",w.innerHTML=`
  ...
  `;const t=e.querySelector("#detail-title"),n=e.querySelector("#detail-meta"),a=e.querySelector("#detail-speakers"),r=e.querySelector("#detail-content"),s=e.querySelector("#export-modal"),o=e.querySelector("#export-options"),c=e.querySelectorAll("[data-tab]");let d=null,u="transcript";e.querySelector("#detail-back").addEventListener("click",i.onBack),c.forEach(l=>{l.addEventListener("click",()=>{u=l.dataset.tab,c.forEach(h=>h.classList.toggle("active",h===l)),d&&w(d)})}),e.querySelector("#detail-export").addEventListener("click",()=>{s.classList.remove("hidden"),s.style.display="flex"}),e.querySelector("#export-cancel").addEventListener("click",()=>{s.classList.add("hidden")}),s.addEventListener("click",l=>{l.target===s&&s.classList.add("hidden")});function p(l){if(l<60)return`${Math.round(l)}s`;const h=Math.floor(l/60);return h<60?`${h} min`:`${Math.floor(h/60)}h ${h%60}min`}function f(l){const h=Math.floor(l/60),m=Math.floor(l%60);return`${h}:${m.toString().padStart(2,"0")}`}function b(l){return P[l%P.length]}function x(l){const h=["var(--accent-coral-light)","var(--badge-indigo-bg)","var(--badge-green-bg)","var(--badge-yellow-bg)"];return h[l%h.length]}function w(l){u==="transcript"?k(l):u==="summary"?I(l):g(l)}function k(l){const h=new Map;l.transcription.speakers.forEach((m,T)=>h.set(m.id,T)),r.innerHTML=l.transcription.segments.map(m=>{const T=h.get(m.speakerId)??0,M=b(T);return`
      ...
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        </div>`),T+="</div>",r.innerHTML=T}function g(l){if(l.actionItems.length===0){r.innerHTML=`
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        `).join("")}
      </div>`}function v(l){if(!d)return;const h=i.exportService.export(d.transcription,l),m=l==="md"?"md":l,T=l==="md"?"text/markdown":l==="vtt"?"text/vtt":"text/plain",M=new Blob([h],{type:T}),C=URL.createObjectURL(M),N=document.createElement("a");N.href=C,N.download=`${d.title.replace(/\s+/g,"_")}.${m}`,N.click(),URL.revokeObjectURL(C),s.classList.add("hidden")}function L(l){d=l,u="transcript",c.forEach(m=>m.classList.toggle("active",m.dataset.tab==="transcript")),t.textContent=l.title,n.innerHTML=`
      <span>${l.date.toLocaleDateString("es")}</span>
      <span>·</span>
      <span>${p(l.duration)}</span>
      <span>·</span>
      <span>${l.transcription.segments.length} segmentos</span>
    `,a.innerHTML=l.transcription.speakers.map((m,T)=>`
    ...
    `).join(""),o.querySelectorAll("button").forEach(m=>{m.addEventListener("click",()=>v(m.dataset.format))}),w(l)}return{element:e,show:L}}function $(i){const e=document.createElement("div");return e.textContent=i,e.innerHTML}async function F(i,e){const t=await z();return new Promise((n,a)=>{const r=t.transaction(i,"readwrite"),o=r.objectStore(i).put(e);o.onsuccess=()=>n(),o.onerror=()=>a(o.error),r.oncomplete=()=>t.close()})}async function K(i){const e=await z();return new Promise((t,n)=>{const a=e.transaction(i,"readonly"),s=a.objectStore(i).getAll();s.onsuccess=()=>t(s.result),s.onerror=()=>n(s.error),a.oncomplete=()=>e.close()})}class A extends Error{constructor(e,t){super(e),this.code=t,this.name="AudioCaptureError"}}class Y{constructor(){this.recordings=new Map}async startRecording(e){let t;try{t=await navigator.mediaDevices.getUserMedia({audio:!0})}catch(c){const d=c instanceof Error?c.message:String(c);throw d.includes("Permission denied")||d.includes("NotAllowedError")?new A("Microphone permission denied by user","PERMISSION_DENIED"):new A(`Microphone not available: ${d}`,"MICROPHONE_NOT_AVAILABLE")}const n=crypto.randomUUID(),a={id:n,startedAt:new Date,source:e.source,status:"recording"},r=new MediaRecorder(t),s=[],o={session:a,config:e,mediaStream:t,mediaRecorder:r,chunks:s,startTimestamp:Date.now()};r.ondataavailable=c=>{c.data.size>0&&s.push(c.data)};for(const c of t.getTracks())c.onended=()=>{o.session.status==="recording"&&(o.session.status="stopped",r.stop())};return r.onerror=()=>{o.session.status="stopped",this.stopTracks(t)},r.start(),this.recordings.set(n,o),{...a}}async stopRecording(e){const t=this.recordings.get(e);if(!t)throw new A(`No recording found for session ${e}`,"SESSION_NOT_FOUND");if(t.session.status==="stopped")throw new A(`Recording ${e} is already stopped`,"ALREADY_STOPPED");const n=await this.finalizeRecorder(t);t.session.status="stopped",this.stopTracks(t.mediaStream);const a=(Date.now()-t.startTimestamp)/1e3,r={id:e,blob:n,duration:a,recordedAt:t.session.startedAt,source:t.config.source,language:t.config.language,syncStatus:"pending"};return await F(E.AUDIO_FILES,r),this.recordings.delete(e),r}getStatus(e){const t=this.recordings.get(e);if(!t)throw new A(`No recording found for session ${e}`,"SESSION_NOT_FOUND");const n=(Date.now()-t.startTimestamp)/1e3;return{isRecording:t.session.status==="recording",duration:n,source:t.config.source}}finalizeRecorder(e){return new Promise(t=>{const{mediaRecorder:n,chunks:a}=e;if(n.state==="inactive"){t(new Blob(a,{type:"audio/webm"}));return}n.onstop=()=>{t(new Blob(a,{type:"audio/webm"}))},n.stop()})}stopTracks(e){for(const t of e.getTracks())t.stop()}}class D extends Error{constructor(e,t){super(e),this.code=t,this.name="STTError"}}class Z{constructor(){this.loaded=!1}async load(){this.loaded=!0}isLoaded(){return this.loaded}async transcribe(e,t,n){const a=n??0;if(a<=0)return[];const r=5,s=Math.max(1,Math.ceil(a/r)),o=[],d={es:["Bienvenidos a la reunión de hoy.","Vamos a revisar los puntos pendientes.","El siguiente tema es importante.","Necesitamos tomar una decisión al respecto.","Perfecto, pasemos al siguiente punto."],en:["Welcome to today's meeting.","Let's review the pending items.","The next topic is important.","We need to make a decision on this.","Great, let's move on to the next point."]}[t];for(let u=0;u<s;u++){const p=u*r,f=Math.min((u+1)*r,a);o.push({startTime:p,endTime:f,text:d[u%d.length],confidence:.75+Math.random()*.2})}return o}}class Q{constructor(e){this.backend=e??new Z}async loadModel(){await this.backend.load()}isReady(){return this.backend.isLoaded()}async transcribe(e,t){if(!this.isReady())throw new D("STT model is not loaded. Call loadModel() first.","MODEL_NOT_LOADED");const n=t??e.language;if(!n)throw new D("Could not determine audio language. Provide a language explicitly.","LANGUAGE_NOT_DETECTED");const a=await this.backend.transcribe(e.blob,n);if(a.length===0)throw new D("Transcription produced no segments — audio may contain no detectable speech.","EMPTY_TRANSCRIPTION");return{segments:a,language:n,duration:e.duration}}}class X extends Error{constructor(e,t){super(e),this.code=t,this.name="DiarizationError"}}const J=.5,R="speaker_unknown",ee="Hablante no identificado";class te{constructor(e=3){this.speakerCount=e}async assignSpeakers(e,t){return t.map((n,a)=>({speakerId:`speaker_${a%this.speakerCount+1}`,confidence:n.confidence}))}}const ne=[/\b(?:hola|buenos días|buenas tardes|buenas noches)?\s*,?\s*soy\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)/i,/\bmi nombre es\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)/i,/\bme llamo\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)/i,/\b(?:hi|hello|hey)?\s*,?\s*I'?m\s+([A-Z][a-z]+)/i,/\bmy name is\s+([A-Z][a-z]+)/i];function ae(i){for(const e of ne){const t=i.match(e);if(t!=null&&t[1])return t[1]}}class re{constructor(e){this.backend=e??new te}async diarize(e,t){if(t.length===0)throw new X("Cannot diarize: no transcription segments provided.","NO_SEGMENTS");const n=await this.backend.assignSpeakers(e,t),a=new Map;let r=1;const s=t.map((d,u)=>{const p=n[u],f=p.confidence<J,b=f?R:p.speakerId,x=p.confidence;!f&&!a.has(b)&&a.set(b,r++);const w=a.get(b),k=f?ee:`Hablante ${w}`;return{...d,speakerId:b,speakerLabel:k,speakerConfidence:x}}),o=new Map;for(const d of s){if(d.speakerId===R)continue;const u=ae(d.text);u&&!o.has(d.speakerId)&&o.set(d.speakerId,u);const p=o.get(d.speakerId);p&&(d.speakerLabel=p)}const c=[];for(const[d,u]of a){const p=o.get(d);c.push({id:d,label:p??`Hablante ${u}`,identifiedName:p})}return{segments:s,speakers:c,language:e.language}}}const se="unassigned",ie="Sin asignar",oe={es:{presupuesto:"Presupuesto",costo:"Costos",costos:"Costos",gasto:"Costos",proyecto:"Proyecto",plazo:"Plazos",plazos:"Plazos",fecha:"Plazos",deadline:"Plazos",diseño:"Diseño",cliente:"Cliente",clientes:"Cliente",equipo:"Equipo",estrategia:"Estrategia",marketing:"Marketing",ventas:"Ventas",producto:"Producto",desarrollo:"Desarrollo",tecnología:"Tecnología",problema:"Problemas",riesgo:"Riesgos",contrato:"Contratos"},en:{budget:"Budget",cost:"Costs",costs:"Costs",expense:"Costs",project:"Project",deadline:"Deadlines",timeline:"Deadlines",schedule:"Deadlines",design:"Design",client:"Client",clients:"Client",team:"Team",strategy:"Strategy",marketing:"Marketing",sales:"Sales",product:"Product",development:"Development",technology:"Technology",issue:"Issues",risk:"Risks",contract:"Contracts"}},ce={es:[/\b(?:hay que|necesitamos|debemos|tenemos que|se debe|se necesita)\s+(.+)/i,/\b(?:me comprometo a|voy a|me encargo de)\s+(.+)/i,/\b(?:por favor|favor)\s+(.+)/i,/\b(?:tarea|acción|pendiente|accionable):\s*(.+)/i,/\b(?:queda pendiente)\s+(.+)/i],en:[/\b(?:we need to|we should|we must|we have to|need to)\s+(.+)/i,/\b(?:I will|I'll|I'm going to|I am going to)\s+(.+)/i,/\b(?:please)\s+(.+)/i,/\b(?:action item|task|todo|to-do):\s*(.+)/i,/\b(?:let's)\s+(.+)/i]},de={es:[/\b(?:se decidió|decidimos|se acordó|acordamos|se aprobó|aprobamos)\s+(.+)/i,/\b(?:la decisión es|la decisión fue)\s+(.+)/i,/\b(?:queda aprobado|queda decidido)\s+(.+)/i,/\b(?:se resolvió|resolvimos)\s+(.+)/i],en:[/\b(?:we decided|it was decided|we agreed|it was agreed)\s+(.+)/i,/\b(?:the decision is|the decision was)\s+(.+)/i,/\b(?:we resolved|it was resolved|we approved)\s+(.+)/i,/\b(?:let's go with|we'll go with)\s+(.+)/i]};class le{extractTopics(e,t){const n=oe[t],a=new Set;for(const r of e){const s=r.text.toLowerCase().split(/\s+/);for(const o of s){const c=o.replace(/[.,;:!?()]/g,"");n[c]&&a.add(n[c])}}return Array.from(a)}extractKeyPoints(e,t){const n=[],a=t==="es"?["importante","clave","decisión","acordamos","conclusión","resumen"]:["important","key","decision","agreed","conclusion","summary"];for(const r of e){const s=r.text.toLowerCase();a.some(o=>s.includes(o))&&n.push(r.text.trim())}return n}detectActionItems(e,t){const n=ce[t],a=[];for(let r=0;r<e.length;r++){const s=e[r].text;for(const o of n){const c=s.match(o);if(c!=null&&c[1]){a.push({description:c[1].trim(),segmentIndex:r});break}}}return a}detectDecisions(e,t){const n=de[t],a=[];for(const r of e)for(const s of n){const o=r.text.match(s);if(o!=null&&o[1]){a.push(o[1].trim());break}}return a}}let pe=1;function ue(){return`action_${pe++}`}class ge{constructor(e){this.backend=e??new le}async generateSummary(e){const{segments:t,language:n}=e,a=this.backend.extractTopics(t,n),r=this.backend.extractKeyPoints(t,n);return a.length===0&&a.push(n==="es"?"Discusión general":"General discussion"),{topics:a,keyPoints:r,language:n}}async generateMinutes(e,t,n){const{segments:a,speakers:r,language:s}=e,o=this.backend.detectDecisions(a,s);return{title:t.topics.length>0?s==="es"?`Acta: ${t.topics[0]}`:`Minutes: ${t.topics[0]}`:s==="es"?"Acta de reunión":"Meeting Minutes",date:new Date,attendees:r,topicsDiscussed:t.topics,decisions:o,actionItems:n,language:s}}async extractActionItems(e){const{segments:t,speakers:n,language:a}=e,r=this.backend.detectActionItems(t,a),s=new Map;for(const o of n)s.set(o.id,o);return r.map(o=>{const c=t[o.segmentIndex],d=c?s.get(c.speakerId):void 0,u=d!==void 0;return{id:ue(),description:o.description,assignedTo:u?d.id:se,assignedToLabel:u?d.identifiedName??d.label:ie,sourceSegmentId:c==null?void 0:c.speakerId}})}}class fe{export(e,t){if(!e||!e.segments)throw new Error("Export failed: transcription data is corrupted or missing segments.");switch(t){case"vtt":return this.exportVTT(e);case"txt":return this.exportTXT(e);case"md":return this.exportMarkdown(e);default:throw new Error(`Unsupported export format: ${t}`)}}importVTT(e){if(!e||typeof e!="string")throw new Error("Import failed: VTT content is empty or invalid.");const t=e.split(`
`);if(!t[0]||!t[0].trim().startsWith("WEBVTT"))throw new Error("Import failed: missing WEBVTT header at line 1.");const n=[],a=new Map;let r=1;for(;r<t.length&&t[r].trim()!=="";)r++;for(;r<t.length;){if(t[r].trim()===""){r++;continue}const c=t[r].trim();if(r++,r>=t.length)throw new Error(`Import failed: unexpected end of file after cue identifier at line ${r}.`);const d=t[r].trim(),u=d.match(/^(\d{2}:\d{2}:\d{2}\.\d{3})\s+-->\s+(\d{2}:\d{2}:\d{2}\.\d{3})$/);if(!u)throw new Error(`Import failed: malformed timestamp at line ${r+1}: "${d}".`);r++;const p=this.parseVTTTimestamp(u[1]),f=this.parseVTTTimestamp(u[2]),b=[];for(;r<t.length&&t[r].trim()!=="";)b.push(t[r].trim()),r++;if(b.length===0)throw new Error(`Import failed: missing cue text after timestamp at line ${r}.`);const x=b.join(" "),w=c.match(/^(speaker_\d+)\s*-\s*(.+)$/);let k,I;w?(k=w[1],I=w[2]):(k=c,I=c),a.has(k)||a.set(k,{id:k,label:I}),n.push({startTime:p,endTime:f,text:x,confidence:1,speakerId:k,speakerLabel:I,speakerConfidence:1})}const s=t[0].match(/Language:\s*(es|en)/i),o=s?s[1]:"es";return{segments:n,speakers:Array.from(a.values()),language:o}}exportVTT(e){const t=`WEBVTT - Language: ${e.language}`,n=e.segments.map(a=>{const r=this.formatVTTTimestamp(a.startTime),s=this.formatVTTTimestamp(a.endTime);return`${`${a.speakerId} - ${a.speakerLabel}`}
${r} --> ${s}
${a.text}`});return[t,"",...n].join(`

`)}exportTXT(e){return e.segments.map(t=>{const n=this.formatTimestamp(t.startTime),a=this.formatTimestamp(t.endTime);return`[${n} - ${a}] ${t.speakerLabel}: ${t.text}`}).join(`
`)}exportMarkdown(e){const t=["# Transcription",""];for(const n of e.segments){const a=this.formatTimestamp(n.startTime),r=this.formatTimestamp(n.endTime);t.push(`**${n.speakerLabel}** _(${a} - ${r})_`),t.push(""),t.push(n.text),t.push("")}return t.join(`
`)}formatVTTTimestamp(e){const t=Math.floor(e/3600),n=Math.floor(e%3600/60),a=Math.floor(e%60),r=Math.round((e-Math.floor(e))*1e3);return String(t).padStart(2,"0")+":"+String(n).padStart(2,"0")+":"+String(a).padStart(2,"0")+"."+String(r).padStart(3,"0")}parseVTTTimestamp(e){const t=e.split(":"),n=parseInt(t[0],10),a=parseInt(t[1],10),r=t[2].split("."),s=parseInt(r[0],10),o=parseInt(r[1],10);return n*3600+a*60+s+o/1e3}formatTimestamp(e){const t=Math.floor(e/3600),n=Math.floor(e%3600/60),a=Math.floor(e%60);return String(t).padStart(2,"0")+":"+String(n).padStart(2,"0")+":"+String(a).padStart(2,"0")}}class me{constructor(){this.audioCapture=new Y,this.exportService=new fe,this.meetings=[],this.currentSessionId=null,this.recordingStartTime=0,this.sttEngine=new Q,this.diarization=new re,this.nlpService=new ge}async init(){await this.sttEngine.loadModel(),await this.loadMeetingsFromDB()}async startRecording(e="es"){const t=await this.audioCapture.startRecording({source:"microphone",language:e});return this.currentSessionId=t.id,this.recordingStartTime=Date.now(),t.id}get activeSessionId(){return this.currentSessionId}async stopAndProcess(e){if(!this.currentSessionId)throw new Error("No active recording session");const t=this.currentSessionId;this.currentSessionId=null;const n=await this.audioCapture.stopRecording(t),a=await this.sttEngine.transcribe(n),r=await this.diarization.diarize(n,a.segments),s=await this.nlpService.generateSummary(r),o=await this.nlpService.extractActionItems(r),c=await this.nlpService.generateMinutes(r,s,o),d={id:n.id,status:"local",transcription:r,audioId:n.id,createdAt:Date.now(),updatedAt:Date.now()};await F(E.TRANSCRIPTIONS,d);const u=e??`Reunión ${new Date().toLocaleString("es")}`,p={id:n.id,title:u,date:n.recordedAt,duration:n.duration,status:"transcribed",transcription:r,summary:s,actionItems:o,minutes:c};return this.meetings.unshift(p),{transcriptionId:n.id,audioFile:n,transcription:r,summary:s,actionItems:o,minutes:c}}getMeetings(){return this.meetings}getMeeting(e){return this.meetings.find(t=>t.id===e)}async loadMeetingsFromDB(){try{const e=await K(E.TRANSCRIPTIONS);for(const t of e){const n=await this.nlpService.generateSummary(t.transcription),a=await this.nlpService.extractActionItems(t.transcription),r=await this.nlpService.generateMinutes(t.transcription,n,a);this.meetings.push({id:t.id,title:`Reunión ${new Date(t.createdAt).toLocaleString("es")}`,date:new Date(t.createdAt),duration:t.transcription.segments.length>0?t.transcription.segments[t.transcription.segments.length-1].endTime:0,status:"transcribed",transcription:t.transcription,summary:n,actionItems:a,minutes:r})}this.meetings.sort((t,n)=>n.date.getTime()-t.date.getTime())}catch{}}}const he=[{id:"home",icon:y.house,label:"Inicio"},{id:"search",icon:y.search,label:"Buscar"},{id:"calendar",icon:y.calendar,label:"Calendario"},{id:"settings",icon:y.settings,label:"Ajustes"}];function xe(){q();const i=new me;i.init().catch(console.error);let e=null;const t=document.createElement("div");t.id="app-shell",t.style.cssText="display:flex;flex-direction:column;min-height:100vh;";const n=H({onRecordClick:()=>w(),onSearchClick:()=>x("search"),onMeetingClick:g=>I(g),getMeetings:()=>i.getMeetings()}),a=U({onBack:()=>k(),onStop:()=>k(),onPause:()=>{},onFlag:()=>{}}),r=W(),s=G({onBack:()=>{n.refresh(),x("home")},exportService:i.exportService}),o=document.createElement("div");o.className="screen",o.style.cssText="padding-bottom:0;",o.innerHTML=`
  ...
  `;const c=document.createElement("div");c.className="screen",c.innerHTML=`
    ...
    </div>`;const u=new Map([["home",n.element],["search",r.element],["calendar",c],["settings",d],["recording",a.element],["processing",o],["detail",s.element]]);u.forEach(g=>t.appendChild(g));const p=document.createElement("nav");p.className="nav-bar",p.setAttribute("role","navigation"),p.setAttribute("aria-label","Navegación principal");const f=new Map;he.forEach(g=>{const v=document.createElement("button");v.className="nav-item",v.type="button",v.innerHTML=`${g.icon}<span>${g.label}</span>`,v.setAttribute("aria-label",g.label),v.addEventListener("click",()=>x(g.id)),f.set(g.id,v),p.appendChild(v)}),t.appendChild(p);const b=["recording","processing","detail"];function x(g){u.forEach((v,L)=>{v.classList.toggle("active",L===g)}),p.style.display=b.includes(g)?"none":"flex",f.forEach((v,L)=>{v.classList.toggle("active",L===g)})}async function w(){try{await i.startRecording("es"),x("recording"),a.reset();let g=0;e=setInterval(()=>{g++,a.updateTimer(g),a.animateWaveform()},1e3)}catch(g){const v=g instanceof Error?g.message:String(g);alert(`No se pudo iniciar la grabación: ${v}`)}}async function k(){if(e&&(clearInterval(e),e=null),!i.activeSessionId){x("home");return}x("processing");try{const g=await i.stopAndProcess(),v=i.getMeeting(g.transcriptionId);v?(s.show(v),x("detail")):(n.refresh(),x("home"))}catch(g){console.error("Processing failed:",g),alert(`Error al procesar la grabación: ${g instanceof Error?g.message:String(g)}`),n.refresh(),x("home")}}function I(g){const v=i.getMeeting(g);v&&(s.show(v),x("detail"))}return x("home"),{root:t,navigateTo:x}}async function ve(){await z(),await O();const i=document.getElementById("app");if(!i)return;const e=xe();i.appendChild(e.root)}ve().catch(console.error);
//# sourceMappingURL=index-BlVVZnjz.js.map
